...
char *NOM_POLICE = "./lib/emulogic.ttf"; // choix d'une police faisant "retro gaming" (pixelisée)
...
void ouvrir_fenetre(int largeur, int hauteur) {
    ...
    if (!f) {
        fprintf(stderr,
                "Erreur: le chemin '%s' ne permet pas de trouver le dossier 'lib' et la police 'emulogic.ttf'\n",
                NOM_POLICE);
        exit(1);
    }
    fclose(f);
...
void dessiner_cercle(Point centre, int rayon, Couleur couleur) {
    ...
    for (p.x = xmin; p.x <= xmax; p.x++) {
        for (p.y = ymin; p.y <= ymax; p.y++)
            if ((centre.x - p.x) * (centre.x - p.x) + (centre.y - p.y) * (centre.y - p.y) <=
                rayon * rayon + rayon / RATIO &&
                (centre.x - p.x) * (centre.x - p.x) + (centre.y - p.y) * (centre.y - p.y) >=
                rayon * rayon - rayon / RATIO)
                changer_pixel(p, couleur);
    }
}
...
// nom est une chaine de caracteres qui est le nom (complet) du fichier image
// coin est le coin haut, gauche voulu pour l'image à afficher dans l'ecran
// en remplaçant une couleur RGB par du transparent

void afficher_image_transparent(char *nom, Point coin, int couleur[]) {
    SDL_Surface *imgcharge = SDL_LoadBMP(nom);
    SDL_SetColorKey(imgcharge, SDL_SRCCOLORKEY, SDL_MapRGB(imgcharge->format, couleur[0], couleur[1], couleur[2]));
    SDL_Rect position_img;
    position_img.x = coin.x;
    position_img.y = coin.y;
...
// récupérer un pointeur qui cible les évènements
Trace_evts *recuperer_pointeur_evenements() {
    return &trace_evts;
}
